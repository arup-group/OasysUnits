...
namespace OasysUnits
{
    ...
    [DataContract]
    public readonly partial struct LapseRate :
        IArithmeticQuantity<LapseRate, LapseRateUnit, double>,
        IComparable,
        IComparable<LapseRate>,
        IConvertible,
        IEquatable<LapseRate>,
        IFormattable
    {
        /// <summary>
        ...
        static LapseRate()
        {
            ...
            Info = new QuantityInfo<LapseRateUnit>("LapseRate",
                ...
                {
                    new UnitInfo<LapseRateUnit>(LapseRateUnit.DegreeCelsiusPerKilometer, "DegreesCelciusPerKilometer", BaseUnits.Undefined, "LapseRate"),
                },
        ...
        /// <inheritdoc cref="TryParseUnit(string,IFormatProvider,out OasysUnits.Units.LapseRateUnit)"/>
        ...
        /// <summary>Indicates strict equality of two <see cref="LapseRate"/> quantities, where both <see cref="Value" /> and <see cref="Unit" /> are exactly equal.</summary>
        [Obsolete("For null checks, use `x is null` syntax to not invoke overloads. For equality checks, use Equals(LapseRate other, LapseRate tolerance) instead, to check equality across units and to specify the max tolerance for rounding errors due to floating-point arithmetic when converting between units.")]
        public static bool operator ==(LapseRate left, LapseRate right)
        {
            return left.Equals(right);
        }

        /// <summary>Indicates strict inequality of two <see cref="LapseRate"/> quantities, where both <see cref="Value" /> and <see cref="Unit" /> are exactly equal.</summary>
        [Obsolete("For null checks, use `x is null` syntax to not invoke overloads. For equality checks, use Equals(LapseRate other, LapseRate tolerance) instead, to check equality across units and to specify the max tolerance for rounding errors due to floating-point arithmetic when converting between units.")]
        public static bool operator !=(LapseRate left, LapseRate right)
        {
            return !(left == right);
        ...
        /// <inheritdoc />
        /// <summary>Indicates strict equality of two <see cref="LapseRate"/> quantities, where both <see cref="Value" /> and <see cref="Unit" /> are exactly equal.</summary>
        [Obsolete("Use Equals(LapseRate other, LapseRate tolerance) instead, to check equality across units and to specify the max tolerance for rounding errors due to floating-point arithmetic when converting between units.")]
        public override bool Equals(object? obj)
        {
            if (obj is null || !(obj is LapseRate otherQuantity))
                return false;
        ...
        /// <summary>Compares the current <see cref="LapseRate"/> with another object of the same type and returns an integer that indicates whether the current instance precedes, follows, or occurs in the same position in the sort order as the other when converted to the same unit.</summary>
        ...
        public int CompareTo(object? obj)
        {
            ...
            if (!(obj is LapseRate otherQuantity)) throw new ArgumentException("Expected type LapseRate.", nameof(obj));

            return CompareTo(otherQuantity);
        }

        /// <summary>Compares the current <see cref="LapseRate"/> with another <see cref="LapseRate"/> and returns an integer that indicates whether the current instance precedes, follows, or occurs in the same position in the sort order as the other when converted to the same unit.</summary>
        ...
        {
            return other is LapseRate otherTyped
                   && (tolerance is LapseRate toleranceTyped
                       ? true
                       : throw new ArgumentException($"Tolerance quantity ({tolerance.QuantityInfo.Name}) did not match the other quantities of type 'LapseRate'.", nameof(tolerance)))
                   && Equals(otherTyped, toleranceTyped);
        }

        /// <inheritdoc />
        public bool Equals(LapseRate other, LapseRate tolerance)
        {
            return OasysUnits.Comparison.Equals(
                referenceValue: this.Value,
                otherValue: other.As(this.Unit),
        ...
        /// <returns>True if successful, otherwise false.</returns>
        private bool TryToUnit(LapseRateUnit unit, [NotNullWhen(true)] out LapseRate? converted)
        {
            if (Unit == unit)
            {
                converted = this;
            ...
            LapseRate? convertedOrNull = (Unit, unit) switch
            {
        ...
        {
            if (unit is not LapseRateUnit typedUnit)
                throw new ArgumentException($"The given unit is of type {unit.GetType()}. Only {typeof(LapseRateUnit)} is supported.", nameof(unit));
