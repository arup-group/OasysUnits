...
{
    /// <summary>
    /// Base converter for serializing and deserializing OasysUnits types to and from JSON.
    /// Contains shared functionality used by <see cref="OasysUnitsIQuantityJsonConverter"/> and <see cref="OasysUnitsIComparableJsonConverter"/>
    /// </summary>
    ...
    public abstract class OasysUnitsBaseJsonConverter<T> : JsonConverter<T>
    {
        ...
        /// <exception cref="OasysUnitsException">Thrown when an invalid Unit has been provided</exception>
        ...
        private Enum GetUnit(string unit)
        {
            ...
            if (unitEnumType is null)
            {
                ...
                var unitEnumTypeAssemblyQualifiedName = "OasysUnits.Units." + unitEnumTypeName + ",OasysUnits";
        ...
        private static (string EnumName, string EnumValue) SplitUnitString(string unit)
        {
            ...
            {
                var ex = new OasysUnitsException($"\"{unit}\" is not a valid unit.");
                ex.Data["type"] = unit;
                throw ex;
            }
