...
{
    /// <summary>
    ///     JSON.net converter for all <see cref="IQuantity" /> types (e.g. all units in OasysUnits)
    ///     Use this converter to serialize and deserialize OasysUnits types to and from JSON using the unit abbreviation schema.
    /// </summary>
    /// <example>
    /// <code>
    /// {
    ...
    public class AbbreviatedUnitsConverter : JsonConverter<IQuantity>
    {
        ...
        ///     otherwise
        /// </returns>
        /// <exception cref="OasysUnitsException">Quantity not found exception is thrown if no match found</exception>
        protected QuantityInfo GetQuantityInfo(string quantityName)
        {
            if (!TryGetQuantity(quantityName, out var quantityInfo))
            {
                throw new OasysUnitsException($"Failed to find the quantity type: {quantityName}.") { Data = { ["type"] = quantityName } };
            }
